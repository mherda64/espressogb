...
public class InstructionBuilder {
    ...
    public InstructionBuilder loadReg(RegEnum reg) {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> registers.get(reg));
        return this;
    }

    public InstructionBuilder loadAddress() {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> addressSpace.get(accumulator) & 0xFF);
        return this;
    }

    public InstructionBuilder add(Integer value) {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> (accumulator + value) & 0xFFFF);
        return this;
    }
    ...
    public InstructionBuilder toSigned() {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> (byte) accumulator);
        return this;
    }
            ...
            switch (reg.size) {
                ...
                case RegEnum.DOUBLE:
                    ...
                    if (isSigned) {
                        ...
                        flags.setCFlag(((regVal & 0xFF) + (accumulator & 0xFF)) > 0xFF);
                    } else {
    ...
    public InstructionBuilder cp(RegEnum reg) {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> sub(registers.get(reg), accumulator, 0, registers));
        return this;
    }
    ...
    public InstructionBuilder setFlag(int flag, boolean b) {
        operations.add((registers, addressSpace, accumulator, context, interruptManager) -> {
            switch (flag) {
                case FlagsReg.Z_FLAG:
                    registers.getFlags().setZFlag(b);
                    break;
                default:
